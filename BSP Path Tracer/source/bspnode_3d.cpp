...
{																																								//	begin
	...
	else																																					//	otherwise, the ray originates on the plane_3d
	{																																							//	begin
...
//------------------------------------------------------------------------------
bool	bspnode::PlaneSearch (const ray &r, polyptr &poly_hit, point_3d &pt) const//	find the intersection of a ray and the plane_3d list
{																																								//	begin
	real	dist = plane.RayIntersection (r);																				//	compute the distance along the ray to the plane_3d
	if (dist > R(0.0))																														//	if it is positive
	{																																							//	begin
		pt = r.IntersectionPoint (dist);																						//	compute the intersection point_3d
		if (on->BoundingBox ().Contains (pt))																				//	check to see if it is in the bounding box for the list
		{																																						//	begin
			iterator	entry (on);																											//	list iterator
			for (polyptr poly = entry (); poly (); poly = entry ())										//	do for all the entries in th elist
				if (((poly->Plane () | r.Direction ()) < R(0.0)) && poly->Contains (pt))//	if the point_3d is inside the polygon
				{																																				//	begin
...
//------------------------------------------------------------------------------
listptr	bspnode::TreeToList (void)																							//	return a list of all the polygons in the tree
{																																								//	begin
	listptr	myList,																																//	place to get a list
					tmpList;																															//	holder for subtree conversions
	myList->Copy (on);																														//	copy the on list to this list
	tmpList = in.TreeToList ();																										//	convert the in subtree to a list
	myList->Append (tmpList);																											//	append it to this list
	tmpList = out.TreeToList ();																									//	convert the out subtree to a list
	myList->Append (tmpList);																											//	append it to this list
	return myList;																																//	return the resulting list
}																																								//	end
