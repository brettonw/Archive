...
SubdivideTriangle (TChain<Real>& chain, TriangleVec& trianglesOut, const Triangle& triangle, Real rMaxEdgeLength)
{
    ...
    if (rLongest > rMaxEdgeLength)
    {
        ...
        // XXX most likely to be close to the end because we added it. It 
        // XXX might be better to use a "new points list" and an offset that
        // XXX has to be added to that - that way I will only search in the
        // XXX new points.
        uInt            iNewPtIndex;
        for (iNewPtIndex = static_cast<uInt> (chain.size () - 1); iNewPtIndex != static_cast<uInt> (-1); --iNewPtIndex)
        ...
        if (iNewPtIndex == static_cast<uInt> (-1))
        {
            ...
            iNewPtIndex = static_cast<uInt> (chain.size ());
